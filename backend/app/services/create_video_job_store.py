"""In-memory job store for the Sutui create-video pipeline."""
...
def create_video_job(*, user_id: int, payload: Dict[str, Any], job_id: Optional[str] = None) -> str:
    jid = (job_id or "").strip().lower()
...
def get_create_video_job(job_id: str) -> Optional[Dict[str, Any]]:
    jid = (job_id or "").strip().lower()
    if not _valid_job_id(jid):
        return None
    now = time.time()
...
def update_create_video_job(job_id: str, **fields: Any) -> bool:
    jid = (job_id or "").strip().lower()
    if not _valid_job_id(jid):
        return False
    with JOBS_LOCK:
...
def append_create_video_progress(job_id: str, *, stage: str, message: str, extra: Optional[Dict[str, Any]] = None) -> bool:
    jid = (job_id or "").strip().lower()
    if not _valid_job_id(jid):
        return False
    with JOBS_LOCK:
