...
GOAL_VIDEO_RE = _rx(r"(目标成片|根据.{0,8}记忆.{0,20}(生成|做|制作|创建).{0,12}(视频|短片|短视频|宣传片|成片)|用.{0,6}记忆.{0,20}(视频|短片|短视频|宣传片|成片))")
...
def classify_openclaw_tool_scope(messages: Sequence[Dict]) -> OpenClawToolScope:
    ...
    if wants_goal_video:
        ...
        return _scope(
            ...
            "用户明确要“目标成片”或“根据记忆生成视频”时，优先直接调用 goal.video.pipeline，payload.action=start_pipeline，payload.goal 必填并直接使用用户原话或整理后的明确目标，不要反问视频主题。不要因为素材库没有产品图而停止；没有参考素材时 reference_asset_ids/reference_image_urls 留空，由流水线先根据记忆生成图片再生成视频。若返回 status=running/openclaw_async/next_payload，只能说明任务仍在运行并按 next_payload 继续 poll_pipeline；禁止说目标成片不可用，禁止改用普通 image.generate/video.generate 作为替代。",
        )
