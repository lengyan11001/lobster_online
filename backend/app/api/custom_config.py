...
@router.get("/api/custom-configs", summary="List all custom config blocks（本地）")
def list_configs(current_user: _ServerUser = Depends(get_current_user_for_local)):
    data = _load_configs()
...
@router.post("/api/custom-configs", summary="Add or update a config block（本地）")
def save_config(body: ConfigBlockIn, current_user: _ServerUser = Depends(get_current_user_for_local)):
...
@router.delete("/api/custom-configs/{name}", summary="Delete a config block（本地）")
def delete_config(name: str, current_user: _ServerUser = Depends(get_current_user_for_local)):
    data = _load_configs()
    configs = data.get("configs", {})
...
def _write_config_to_env(name: str, config: dict):
    ...
    Note: Some configs (like TOS_CONFIG) should NOT be written to .env
    because they are not part of the Settings class and will cause validation errors.
    These configs should only exist in custom_configs.json.
    """
    # Configs that should NOT be written to .env (only in custom_configs.json)
    _ENV_BLACKLIST = {"TOS_CONFIG", "BILLING_PRICING"}
    
    if name in _ENV_BLACKLIST:
        # Remove existing entries from .env if any
...
def list_custom_models(current_user: _ServerUser = Depends(get_current_user_for_local)):
    data = _load_configs()
    return {"models": data.get("custom_models", [])}


@router.post("/api/custom-models", summary="Add a custom model")
def add_custom_model(body: CustomModelIn, current_user: _ServerUser = Depends(get_current_user_for_local)):
...
def delete_custom_model(model_id: str, current_user: _ServerUser = Depends(get_current_user_for_local)):
    data = _load_configs()
