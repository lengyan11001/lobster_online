...
from .openclaw_chat_gateway import (
...
def _refresh_remote_generation_config() -> None:
    """Fetch generation defaults and Comfly model IDs from auth server, cached with TTL."""
    global _comfly_image_models_cache, _comfly_image_models_ts
    global _remote_image_generate_default_model_cache, _remote_video_generate_default_model_cache
    now = time.time()
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if isinstance(defaults, dict):
                    image_default = str(defaults.get("image_generate_model") or "").strip()
                    video_default = str(defaults.get("video_generate_model") or "").strip()
                    _remote_image_generate_default_model_cache = image_default
                    _remote_video_generate_default_model_cache = video_default
                _comfly_image_models_cache = result
                _comfly_image_models_ts = now
                logger.info(
                    "[CHAT] remote generation config refreshed: default_image_model=%s default_video_model=%s comfly_models=%s",
                    _remote_image_generate_default_model_cache or "-",
                    _remote_video_generate_default_model_cache or "-",
                    result,
                )
                return
            else:
...
def _enrich_saved_assets_asset_ids_from_db(
    ...
    from .assets import (
        _find_existing_asset_by_save_url_dedupe,
        _save_url_dedupe_key,
        _save_url_prompt_is_placeholder,
    )
    from ..models import Asset

    changed = False

    def _backfill_prompt_model(row: Any, item: Dict[str, Any]) -> None:
        nonlocal changed
        if row is None or not isinstance(item, dict):
            return
        prompt = str(item.get("prompt") or "").strip()
        model = str(item.get("model") or "").strip()
        if prompt and _save_url_prompt_is_placeholder(getattr(row, "prompt", None)):
            row.prompt = prompt[:500]
            changed = True
        if model and not (getattr(row, "model", None) or "").strip():
            row.model = model[:128]
            changed = True
    ...
    for it in saved:
        if not isinstance(it, dict):
            continue
        aid0 = (it.get("asset_id") or "").strip()
        if aid0:
            row0 = (
                db.query(Asset)
                .filter(Asset.user_id == int(user_id), Asset.asset_id == aid0)
                .first()
            )
            _backfill_prompt_model(row0, it)
            continue
    ...
    if changed:
        try:
            db.commit()
            logger.info("[CHAT] saved_assets prompt/model 已回填到素材库")
        except Exception as e:
            db.rollback()
            logger.warning("[CHAT] saved_assets prompt/model 回填素材库失败: %s", e)


def _publish_copy_source_prompt_from_context(pctx: Optional[Dict[str, Any]]) -> str:
    if not pctx:
        return ""
    msg = ""
    for m in reversed(pctx.get("messages") or []):
        ...
        if msg:
            break
    text = _strip_lobster_attachment_block(msg)
    if not text:
        return ""
    text = _IMAGE_PUBLISH_ACTION_RE.sub("", text)
    text = _IMAGE_SOCIAL_ACCOUNT_RE.sub("", text)
    text = re.sub(r"(?:视频|图片)?\s*时长\s*[:：]?\s*\d+(?:\.\d+)?\s*(?:秒|s|S)?", "", text)
    text = re.sub(r"\b\d+(?:\.\d+)?\s*(?:秒|s|S)\b", "", text)
    text = re.sub(r"\s+", " ", text)
    text = text.strip(" \t\r\n，,。.;；:：-_/|")
    return text[:500]


def _attach_publish_copy_source_prompt_from_context(
    args: Dict[str, Any],
    db: Optional[Session],
    ...
    if (opts.get("_source_prompt") or "").strip():
        return
    aid = (args.get("asset_id") or "").strip()
    if aid and db is not None and user_id is not None:
        try:
            from ..models import Asset

            row = (
                db.query(Asset)
                .filter(Asset.user_id == int(user_id), Asset.asset_id == aid)
                .first()
            )
            if row is not None and (getattr(row, "prompt", None) or "").strip():
                return
        except Exception:
            pass
    prompt = _publish_copy_source_prompt_from_context(pctx)
    if not prompt:
        return
    opts = dict(opts)
    opts["_source_prompt"] = prompt
    args["options"] = opts
    logger.info("[CHAT] publish_content 已附加发布文案 source_prompt=%s", prompt[:120])
...
def _build_lobster_main_system_prompt(edition: str, has_tools: bool) -> str:
    ...
    if has_tools:
        ...
        default_video_model = _get_default_video_generate_model()
        body += (
            "【视频模型】用户指定模型时必须原样传入 payload.model。"
            f"用户未指定时默认使用服务器下发的视频模型 {default_video_model}。"
            "文生视频和图生视频都先使用该默认视频模型；禁止自行替换为 seedance、sora2 或其它模型。\n"
        )
    else:
        body = _no_tools_sys_hint(edition)
...
def _correct_image_to_video_or_tvc_if_user_asked(args: Dict, last_user_content: str) -> Dict:
    ...
    if image_url:
        ...
        new_payload["images"] = image_urls
        new_payload["image_files"] = image_urls
    elif asset_id:
...
def _correct_generation_to_understand_if_user_asked(
    ...
    urls = _attachment_urls_for_media_type(last_user_content, media_type, attachment_urls)
    old_payload = args.get("payload") if isinstance(args.get("payload"), dict) else {}
    for key in ("video_urls", "image_urls", "media_files", "filePaths", "images", "image_files"):
        val = old_payload.get(key)
...
_MEDIA_EDIT_TEXT_REQUEST_RE = re.compile(
    r"(?:加|添加|配|写|写上|打上|放上|显示|叠).{0,16}(?:字幕|字慕|文字|文案|标题|字)|"
    r"(?:字幕|字慕|文字|文案|标题).{0,8}(?:加到|放到|显示到|叠到)",
    re.IGNORECASE,
)
_MEDIA_EDIT_TEXT_VALUE_RE = re.compile(
    r"(?:字幕文案|字慕文案|字母文案|字幕|字慕|文字|文案|标题)\s*[:：]\s*([^\n\r【]+)",
    re.IGNORECASE,
)
_MEDIA_EDIT_ASSET_ID_RE = re.compile(
    r"(?:素材\s*ID|asset[_\s-]*id)\s*[·:：=\-]?\s*([a-zA-Z0-9]{10,64})",
    re.IGNORECASE,
)


def _extract_media_edit_asset_id_from_turn(
    user_message: str,
    attachment_asset_ids: Optional[List[str]] = None,
) -> str:
    text = user_message or ""
    for m in _MEDIA_EDIT_ASSET_ID_RE.finditer(text):
...
def _extract_media_edit_overlay_text(user_message: str) -> str:
    text = _strip_lobster_attachment_block(user_message)
    picked = ""
    for m in _MEDIA_EDIT_TEXT_VALUE_RE.finditer(text or ""):
...
def _ensure_media_edit_payload_from_current_turn(
    args: Dict[str, Any],
    last_user_content: str,
    attachment_asset_ids: Optional[List[str]] = None,
) -> Dict[str, Any]:
    """LLM 已选 media.edit 但漏 operation/asset_id 时，按当前轮明确剪辑意图补齐。"""
    if not args or (args.get("capability_id") or "").strip() != "media.edit":
        return args
    user_text = _strip_lobster_attachment_block(last_user_content)
    if not (user_text and _MEDIA_EDIT_TEXT_REQUEST_RE.search(user_text)):
        return args

    payload = args.get("payload") if isinstance(args.get("payload"), dict) else {}
    payload = dict(payload)
    for key in ("operation", "asset_id", "text", "position", "vertical_align", "font_size", "font_color"):
        if key in args and args.get(key) is not None and not payload.get(key):
            payload[key] = args.get(key)
    changed = False
    if not (payload.get("operation") or "").strip():
        payload["operation"] = "overlay_text"
        changed = True
    if payload.get("operation") != "overlay_text":
        args["payload"] = payload
        return args

    if not (payload.get("asset_id") or "").strip():
        aid = _extract_media_edit_asset_id_from_turn(last_user_content, attachment_asset_ids)
        if aid:
            payload["asset_id"] = aid
            changed = True
    if not (payload.get("text") or "").strip():
        overlay_text = _extract_media_edit_overlay_text(last_user_content)
        if overlay_text:
            payload["text"] = overlay_text
            changed = True
    if not (payload.get("position") or payload.get("vertical_align")):
        if re.search(r"(下方|底部|下面|下边|底端)", user_text):
            payload["position"] = "bottom"
            changed = True
        elif re.search(r"(中间|中央|居中)", user_text):
            payload["position"] = "center"
            changed = True
        elif re.search(r"(上方|顶部|上面|上边|顶端)", user_text):
            payload["position"] = "top"
            changed = True
    if changed:
        args["payload"] = payload
        logger.warning(
            "[CHAT] media.edit 参数缺失，已按当前用户文本补全 operation=%s asset_id=%s text=%s",
            payload.get("operation") or "-",
            payload.get("asset_id") or "-",
            str(payload.get("text") or "")[:80] or "-",
        )
    return args
...
def _correct_daihuo_pipeline_to_video_if_plain_video(args: Dict[str, Any], user_message: str) -> Dict[str, Any]:
    ...
    if image_url:
        ...
        new_payload["image_url"] = asset_id
    for key in ("media_files", "filePaths", "images", "image_files"):
        val = old_payload.get(key)
        if val:
            new_payload[key] = val
...
def _inject_video_media_urls(args: Dict[str, Any], attachment_urls: List[str]) -> None:
    ...
    if not attachment_urls:
        if not inner.get("media_files") and not inner.get("image_url") and not inner.get("filePaths") and not inner.get("images") and not inner.get("image_files"):
            logger.warning(
    ...
    logger.info("[CHAT] 图生视频注入 filePaths/images（%d 张）functionMode=first_last_frames", len(urls))
...
def _infer_video_model_from_user_text(
    ...
    if not text:
        return
    low = text.lower()
    provider_model = _VIDEO_PROVIDER_MODEL_ID_RE.search(text)
    if provider_model:
        inner["model"] = provider_model.group(1).strip()
        logger.info(
            "[CHAT] user text contains provider video model id, filled payload.model=%s",
            inner["model"],
        )
        return
    ...
    for needle, canonical in _VEO_FULL_MODEL_SUBSTR:
        if needle in low:
            ...
            return
    if _VEO_31_FAST_IN_USER_TEXT.search(text):
        inner["model"] = "veo3.1-fast"
        logger.info(
            "[CHAT] 用户正文含 Veo 3.1 Fast 但 payload 无 model，已补全 model=%s",
            inner["model"],
        )
        return
...
_VIDEO_MODEL_DIRECTIVE_RE = re.compile(
    r"(?:请|帮我|用|使用|选择|指定)?\s*"
    r"(?:fal-ai|st-ai|sora2pub|ark|wan|xai|sprcra|minimax)/[A-Za-z0-9._~:+/-]+"
    r"\s*(?:模型|model)?\s*(?:来|去)?\s*(?:生成|做|制作)?\s*(?:一个|一段)?\s*(?:视频)?",
    re.IGNORECASE,
)
_VIDEO_SHORT_MODEL_DIRECTIVE_RE = re.compile(
    r"(?:请|帮我|用|使用|选择|指定)\s*"
    r"(?:veo\s*3(?:[\._]?\s*1)?(?:\s*[-_]?\s*fast)?|veo3\.1-fast|super[-_\s]*seed2(?:[-_\s]*lite)?)"
    r"\s*(?:模型|model)?\s*(?:来|去)?\s*(?:生成|做|制作)?\s*(?:一个|一段)?\s*(?:视频)?",
    re.IGNORECASE,
)


def _strip_video_model_directives_from_prompt(prompt: str) -> str:
    raw = str(prompt or "").strip()
    if not raw:
        return raw
    cleaned = _VIDEO_MODEL_DIRECTIVE_RE.sub("", raw)
    cleaned = _VIDEO_SHORT_MODEL_DIRECTIVE_RE.sub("", cleaned)
    cleaned = re.sub(r"([:：])\s*[,，、;；.。]\s*", r"\1", cleaned)
    cleaned = re.sub(r"\s+([,，.。;；:：])", r"\1", cleaned)
    cleaned = re.sub(r"([,，;；])\s*([,，;；])+", r"\1", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip(" \t\r\n，,。.;；:：-_/|")
    return cleaned if len(_compact_image_prompt_text(cleaned)) >= 4 else raw
...
def _sanitize_video_generate_prompt_for_publish_copy(prompt: str, last_user_content: str) -> str:
    raw = _strip_video_model_directives_from_prompt(str(prompt or "").strip())
    if not raw:
        return raw
    user_text = _strip_lobster_attachment_block(last_user_content)
    if not (_IMAGE_PUBLISH_CONTEXT_RE.search(raw) or _IMAGE_PUBLISH_CONTEXT_RE.search(user_text or "")):
...
def _ensure_video_generate_prompt_source_and_clean_publish_context(
    ...
    if cleaned_prompt and cleaned_prompt != prompt:
        ...
        logger.warning(
            "[CHAT-ORCH] video.generate prompt 含控制/发布指令，已清洗 old=%s new=%s",
            prompt[:160] if prompt else "-",
            cleaned_prompt[:160],
        )

    image_fields_present = any(inner.get(k) for k in ("image_url", "media_files", "filePaths", "images", "image_files"))
    ...
    if image_fields_present and not references_image:
        removed = {}
        for key in ("image_url", "media_files", "filePaths", "images", "image_files", "functionMode"):
            if key in inner:
    ...
    route = "image_to_video" if any(inner.get(k) for k in ("image_url", "media_files", "filePaths", "images", "image_files")) else "text_to_video"
    logger.info(
...
def _ensure_video_generate_default_model(args: Dict[str, Any], user_message: str = "") -> None:
    """video.generate 使用系统默认模型；用户显式指定或延续上一轮时保留 LLM 传入的 model。"""
    if not args or (args.get("capability_id") or "").strip() != "video.generate":
        return
    inner = args.get("payload")
    if not isinstance(inner, dict):
    ...
    if not has_img:
        for key in ("media_files", "filePaths", "images", "image_files"):
            ...
            if isinstance(vals, list) and any(str(v or "").strip() for v in vals):
                has_img = True
                break
    chosen = _get_default_video_generate_model(has_img)
    user_named_model = _user_explicitly_requested_video_model(user_message)
    user_continues_previous = bool(_VIDEO_HISTORY_CONTINUATION_RE.search(_strip_lobster_attachment_block(user_message) or ""))
    if raw_model and not user_named_model and not user_continues_previous:
        if raw_model != chosen:
            logger.info(
    ...
    if dur_f is None or dur_f <= 0:
        ...
        if inferred_duration:
            inner["duration"] = inferred_duration
            logger.info(
                "[CHAT] video.generate 未传 duration，已从用户原话补全 duration=%s",
                inferred_duration,
            )
        else:
            inner.pop("duration", None)
            logger.info("[CHAT] video.generate 未传 duration，保留为空，由 MCP 按模型默认值处理")
...
def _invoke_model_for_cost_confirm(capability_id: Optional[str], args: Dict[str, Any]) -> Optional[str]:
    ...
    if capability_id in ("image.generate", "video.generate"):
        ...
        return m or None
    if capability_id == "comfly.daihuo":
...
def _ensure_understand_default_model(args: Dict[str, Any]) -> None:
    """理解类能力只使用当前已验证的默认视觉/视频理解模型，避免 LLM 填入未开通模型。"""
    if not args:
        return
    cap = (args.get("capability_id") or "").strip()
    if cap not in ("image.understand", "video.understand"):
    ...
    if not isinstance(inner, dict):
        inner = {}
        args["payload"] = inner
    default_model = "openrouter/router/video" if cap == "video.understand" else "openrouter/router/vision"
    raw = (inner.get("model") or "").strip()
    if raw and raw != default_model:
        logger.warning("[CHAT] %s model=%s 非默认理解模型，已回落为 %s", cap, raw, default_model)
    if raw != default_model:
        inner["model"] = default_model


def _resolve_understand_payload_asset_ids_to_urls(
    args: Dict[str, Any],
    request: Optional[Request],
    db,
    user_id: Optional[int],
) -> None:
    """image/video.understand：LLM 只传 asset_id 时，先解析为上游必需的 image_urls/video_urls。"""
    if not args:
        return
    cap = (args.get("capability_id") or "").strip()
    if cap not in ("image.understand", "video.understand"):
    ...
    if not isinstance(inner, dict):
        inner = {}
        args["payload"] = inner
    media_key = "video_urls" if cap == "video.understand" else "image_urls"
    single_key = "video_url" if cap == "video.understand" else "image_url"
    if inner.get(media_key) or inner.get(single_key):
        return
    aid = str(inner.get("asset_id") or inner.get("media_asset_id") or "").strip()
    if not aid or not _VIDEO_PAYLOAD_ASSET_ID_TOKEN.match(aid):
        return

    from .assets import get_asset_public_url

    u = get_asset_public_url(aid.lower(), user_id, request, db)
    if not u:
        asb = (getattr(settings, "auth_server_base", None) or "").strip().rstrip("/") or "（未配置 AUTH_SERVER_BASE）"
        raise HTTPException(
            status_code=400,
            detail=(
                f"理解素材参数中的素材 ID「{aid}」无法解析为公网 URL。请重新上传该素材，"
                f"或确认素材库 source_url（TOS / 服务器临时上传）有效；AUTH_SERVER_BASE={asb}。"
            ),
        )
    if _LOCAL_SIGNED_ASSET_PATH in u:
        raise HTTPException(status_code=400, detail=f"素材 {aid} 为旧版签名链接，已不可用。请重新上传后重试。")
    inner[media_key] = [u]
    logger.info("[CHAT] %s 已将 asset_id 解析为公网 URL asset_id=%s", cap, aid)
...
def _normalize_invoke_daihuo_pipeline_args_for_chat(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(args, dict):
        return args
    if (args.get("capability_id") or "").strip() != "comfly.daihuo.pipeline":
        return args
    raw_pl = args.get("payload")
    pl: Dict[str, Any] = dict(raw_pl) if isinstance(raw_pl, dict) else {}
    nested = pl.get("payload")
...
async def _chat_daihuo_inline_start_and_poll(
    ...
    if sr.status_code >= 400:
        ...
        return (
            json.dumps(
                {"capability_id": "comfly.daihuo.pipeline", "ok": False, "error": str(detail)},
                ensure_ascii=False,
                indent=2,
            ),
            False,
...
def _parse_daihuo_pipeline_tool_result(result_text: str) -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(d, dict):
        return None
    if (d.get("capability_id") or "").strip() != "comfly.daihuo.pipeline":
        return None
    return d
...
async def _exec_tool(
    ...
    if capability_id == "video.generate":
        ...
        phase = "task_polling"
    elif capability_id == "comfly.daihuo":
    ...
    elif name == "invoke_capability" and (args.get("capability_id") or "").strip() == "comfly.daihuo.pipeline":
        # MCP 在同一次 tools/call 内轮询整包任务最久约 2h（mcp/http_server _COMFLY_DAIHUO_MCP_POLL_MAX_SEC）；
        # 对话层默认 120s 会先 ReadTimeout，用户只看到「失败且无详情」。
        timeout = 130 * 60.0
    elif name == "invoke_capability" and (args.get("capability_id") or "").strip() == "comfly.daihuo":
        pl_cv = args.get("payload") if isinstance(args.get("payload"), dict) else {}
    ...
    if name in ("publish_content", "list_publish_accounts") and not _schedule_orchestration_active.get():
        ...
        if pctx:
            ...
            for _m in reversed(pctx.get("messages") or []):
                if _m.get("role") == "user" and isinstance(_m.get("content"), str):
                    ...
                    break
            if _user_msg and not _PUBLISH_INTENT_RE.search(_strip_lobster_attachment_block(_user_msg)):
        ...
        _attach_publish_copy_source_prompt_from_context(args, db, user_id, pctx)
        if pctx:
    ...
    if name == "invoke_capability" and capability_id == "task.get_result":
        ...
        if tid_veo.startswith("video_"):
            ...
            result_text = (
                "此 task_id 来自 Comfly Veo（comfly.daihuo 的 submit_video），不是速推异步任务。"
                "对速推调用 task.get_result 会报「任务不存在」或类似错误。"
                "请改用：invoke_capability(capability_id=\"comfly.daihuo\", "
                'payload={"action":"poll_video","task_id":"'
                + safe_tid
                + '"}) 轮询直至上游返回完成与视频地址。'
            )
            logger.warning(
                "[对话] 已拦截误用 task.get_result（Comfly Veo task_id=%s），应使用 comfly.daihuo poll_video",
                safe_tid[:96],
            )
    ...
    try:
        ...
        if capability_id == "video.generate":
            ...
            logger.info(
                "[CHAT] 发 MCP video.generate payload: model=%s image_url=%s media_files=%s images=%s",
                (pl.get("model") or "(无)"),
    ...
    if phase == "task_polling":
        if capability_id == "comfly.daihuo":
            ...
            logger.info(
                "[进度] comfly.daihuo poll 单次返回 in_progress=%s",
    ...
    if success and name == "invoke_capability" and capability_id == "comfly.daihuo":
        ...
        if sse_cf:
            ...
            logger.info(
                "[对话] comfly.daihuo poll 终态 tool_end saved_assets 条数=%s task_id=%s",
                len(sse_cf),
                tid_cf or "-",
            )
    if progress_cb:
...
_GENERATE_TERMINAL_RESUBMIT_MAX = 3
_GENERATE_TERMINAL_RESUBMIT_DELAY_SEC = 2.0
...
def _parse_comfly_veo_mcp_tool_result(result_text: str) -> Optional[Dict[str, Any]]:
    """MCP invoke_capability(comfly.daihuo) 文本为 JSON：{"capability_id":"comfly.daihuo","result":{...}}。"""
    ...
    if not isinstance(d, dict):
        return None
    if (d.get("capability_id") or "").strip() != "comfly.daihuo":
        return None
    r1 = d.get("result")
...
def _maybe_register_comfly_veo_submit_hint(result_text: str, invoke_args: Dict[str, Any]) -> None:
    ...
    _register_generation_hint_for_task(tid, {"prompt": prompt, "model": model}, "comfly.daihuo")


def _saved_assets_from_comfly_veo_poll_success(result_text: str) -> List[Dict[str, Any]]:
    """comfly.daihuo poll_video 上游终态且含视频直链时，供 SSE saved_assets + 前端 save-url。"""
    r1 = _parse_comfly_veo_mcp_tool_result(result_text)
    if not r1 or not r1.get("ok", True):
        return []
...
def _should_auto_poll_comfly_veo_after_submit(invoke_args: Dict[str, Any], submit_res: str) -> bool:
    if (invoke_args.get("capability_id") or "").strip() != "comfly.daihuo":
        return False
...
def _comfly_veo_polling_final_progress_event(*, result_text: str, task_id: str) -> Dict[str, Any]:
    """comfly.daihuo 后台多轮 poll 结束后补发 tool_end（与 task.get_result 轮询补发同理）。"""
    res = result_text or ""
    ev: Dict[str, Any] = {
        "type": "tool_end",
        "name": "invoke_capability",
    ...
    if sse:
        ...
        if up is not None:
            ...
            if _comfly_veo_status_is_failed(st):
                ev["success"] = False
    logger.info(
        "[对话轮询] comfly.daihuo 终态补发 tool_end saved_assets 条数=%s task_id=%s",
        len(sse) if sse else 0,
        tid or "-",
    )
    return ev
...
def _extract_generation_failure_reason(result_text: str) -> str:
    ...
    def _walk(obj: Any, depth: int = 0) -> str:
        ...
        for key in ("error", "message", "msg", "detail", "desc", "reason", "fail_reason", "failReason", "failure_reason"):
            val = obj.get(key)
    ...
    m = re.search(
        r'"(?:error|message|msg|detail|desc|reason|fail_reason|failReason|failure_reason)"\s*:\s*"([^"\\]*(?:\\.[^"\\]*)*)"',
        raw,
    )
    if m:
        try:
...
def _is_generation_failure_result(result_text: str) -> bool:
    ...
    if re.search(r'"ok"\s*:\s*false\b', low):
        return True
    if _extract_generation_failure_reason(raw) and re.search(
        r'"(?:error|detail|desc|reason|fail_reason|failReason|failure_reason)"\s*:',
        raw,
        re.IGNORECASE,
    ):
        return True
...
def _is_retryable_generate_terminal_failure(result_text: str) -> bool:
    ...
    try:
        ...
        pass
    if not _is_generation_failure_result(raw):
        return False
...
def _generation_failed_user_reply(capability_id: str, result_text: str, wants_publish: bool = False) -> str:
    ...
    m_retry = re.search(r"\[AUTO_RETRY\]\s*(?:image|video)\.generate\s+resubmit_attempts=(\d+)", result_text or "")
    if m_retry:
        parts.append(f"已自动重试 {m_retry.group(1)} 次仍未成功。")
    if wants_publish:
...
async def _poll_comfly_veo_after_submit(
    ...
    if not tid:
        return submit_res
    logger.info("[CHAT] comfly.daihuo submit 成功，开始自动 poll_video task_id=%s", tid[:96])
    poll_a: Dict[str, Any] = {
        "capability_id": "comfly.daihuo",
    ...
    while waited < max_wait_sec:
        ...
        entered_loop = True
        logger.info("[CHAT] comfly.daihuo poll 轮询 %ds task_id=%s", waited, tid[:96])
...
async def _resume_chat_task_poll_only(
    ...
    if tid.startswith("video_"):
        ...
        while waited < max_wait_sec:
            ...
            waited += poll_interval
            entered_loop = True
            logger.info("[CHAT/stream resume] comfly.daihuo poll %ds task_id=%s", waited, tid[:96])
            if progress_cb:
                try:
                    ev_pre: Dict[str, Any] = {
                        "type": "task_poll",
...
async def _retry_generate_after_retryable_terminal_failure(
    invoke_args: Dict[str, Any],
    initial_final: str,
    token: str,
    sutui_token: Optional[str],
    ...
    media_task_cn = "Veo 任务" if cap == "video.generate" else "图片任务"
    final = initial_final or ""
    attempts_done = 0
    original_task_id = _extract_task_id_from_result(final)
    retry_task_ids: List[str] = []
    for attempt in range(1, _GENERATE_TERMINAL_RESUBMIT_MAX + 1):
        if not _is_retryable_generate_terminal_failure(final):
            break
        attempts_done = attempt
        reason = _extract_generation_failure_reason(final)
        logger.warning(
            "[CHAT] %s 终态失败可重提，自动重试 %s/%s original_task_id=%s reason=%s",
            cap,
            attempt,
            _GENERATE_TERMINAL_RESUBMIT_MAX,
            original_task_id or "-",
            (reason or final)[:300],
        )
        if progress_cb:
            try:
                ev_retry: Dict[str, Any] = {
                    "type": "task_retry",
                    "message": f"{media_task_cn}失败，正在自动重试 {attempt}/{_GENERATE_TERMINAL_RESUBMIT_MAX}…",
                    "capability_id": cap,
                    "attempt": attempt,
                    "max_attempts": _GENERATE_TERMINAL_RESUBMIT_MAX,
                }
                if original_task_id:
                    ev_retry["original_task_id"] = original_task_id
                    ev_retry["task_id"] = original_task_id
                if retry_task_ids:
                    ev_retry["retry_task_ids"] = list(retry_task_ids)
                if reason:
                    ev_retry["result_hint"] = reason[:160]
                await progress_cb(ev_retry)
            except Exception:
                pass
        if _GENERATE_TERMINAL_RESUBMIT_DELAY_SEC > 0:
            await asyncio.sleep(_GENERATE_TERMINAL_RESUBMIT_DELAY_SEC)

        submit_args = copy.deepcopy(invoke_args)
        submit_res = await _exec_tool(
            "invoke_capability",
            submit_args,
            token,
            sutui_token,
            # 自动重提属于同一次用户已确认的生成意图；静默提交，避免每次重试再次弹积分预扣确认。
            progress_cb=None,
            request=request,
            db=db,
            user_id=user_id,
        )
        retry_tid = _extract_task_id_from_result(submit_res)
        if retry_tid:
            retry_task_ids.append(retry_tid)
            pl_retry = submit_args.get("payload") if isinstance(submit_args.get("payload"), dict) else {}
            _register_generation_hint_for_task(retry_tid, pl_retry, cap)
            logger.info(
                "[CHAT] %s 自动重提成功提交 attempt=%s/%s retry_task_id=%s original_task_id=%s",
                cap,
                attempt,
                _GENERATE_TERMINAL_RESUBMIT_MAX,
                retry_tid[:96],
            ...
            if progress_cb:
                try:
                    await progress_cb(
                        {
                            "type": "task_retry",
                            "message": f"已重新提交{media_task_cn}，正在查询结果…（{attempt}/{_GENERATE_TERMINAL_RESUBMIT_MAX}）",
                            "capability_id": cap,
                            "attempt": attempt,
                            "max_attempts": _GENERATE_TERMINAL_RESUBMIT_MAX,
                            "task_id": retry_tid,
                            "original_task_id": original_task_id,
                            "retry_task_ids": list(retry_task_ids),
                        }
                    )
                except Exception:
                    pass
            poll_a = _normalize_invoke_task_get_result_args(
                {
                    "capability_id": "task.get_result",
                    "payload": {"task_id": retry_tid, "capability_id": cap},
                }
            )
            poll_res = await _exec_tool(
                "invoke_capability",
                poll_a,
                token,
                sutui_token,
        ...
        logger.warning(
            "[CHAT] %s 自动重提未拿到 task_id，停止自动重试 preview=%s",
            cap,
            (submit_res or "")[:600],
        )
        final = submit_res or final
        break

    if attempts_done and _is_generation_failure_result(final):
        final = final.rstrip() + f"\n\n[AUTO_RETRY] {cap} resubmit_attempts={attempts_done}"
    return final
...
async def _after_generate_auto_task_result(
    ...
    if cap in ("image.generate", "video.generate"):
        final = await _retry_generate_after_retryable_terminal_failure(
            invoke_args, final, token, sutui_token, progress_cb, request, db=db, user_id=user_id
        )
    if final and '"error"' not in final and not _is_generation_failure_result(final):
...
async def _chat_openai(
    ...
    _veo_only_tools = {"comfly.daihuo", "comfly.daihuo.pipeline"}
    oai_tools = [
        {
            "type": "function",
            "function": {
            ...
            for tc in tcs:
                ...
                if fn.get("name") == "invoke_capability":
                    ...
                    if not _mismatch_err:
                        ...
                        _resolve_understand_payload_asset_ids_to_urls(a, request, db, user_id)
                        _ensure_understand_default_model(a)
            ...
            for tc_info in text_calls:
                ...
                if tc_info["name"] == "invoke_capability":
                    ...
                    if not _mismatch_err_tc:
                        ...
                        _resolve_understand_payload_asset_ids_to_urls(tc_info["arguments"], request, db, user_id)
                        _ensure_understand_default_model(tc_info["arguments"])
            ...
            for tu in tus:
                ...
                if tu.get("name") == "invoke_capability":
                    ...
                    if not _mismatch_err_a:
                        ...
                        _resolve_understand_payload_asset_ids_to_urls(inp, request, db, user_id)
                        _ensure_understand_default_model(inp)
