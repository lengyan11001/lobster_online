...
async def _wechat_get_access_token(appid: str, secret: str) -> str:
    ...
    if not token:
        raise HTTPException(
            status_code=400,
            detail=f"公众号 access_token 获取失败: {data.get('errmsg') or data}",
        )
    return str(token)
...
async def _wechat_upload_bytes(access_token: str, data: bytes, filename: str, permanent: bool) -> str:
    ...
    if permanent:
        ...
        if not media_id:
            raise HTTPException(status_code=400, detail=f"公众号封面上传失败: {resp.get('errmsg') or resp}")
        return str(media_id)
    wx_url = resp.get("url")
    if not wx_url:
        raise HTTPException(status_code=400, detail=f"公众号正文图片上传失败: {resp.get('errmsg') or resp}")
    return str(wx_url)
...
async def _create_wechat_draft(
    ...
    if not media_id:
        raise HTTPException(status_code=400, detail=f"公众号草稿创建失败: {data.get('errmsg') or data}")
    return str(media_id)
