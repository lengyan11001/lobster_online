...
def _start_mcp_if_needed():
    ...
    try:
        ...
        _ed = (getattr(settings, "lobster_edition", None) or "online").strip().lower()
        _mcp_env["LOBSTER_EDITION"] = _ed
        if _ed == "online" and not (os.environ.get("MCP_AUTOSAVE_ASSETS") or "").strip():
            _mcp_env["MCP_AUTOSAVE_ASSETS"] = "1"
        _asb = (getattr(settings, "auth_server_base", None) or "").strip().rstrip("/")
        if _asb:
            _mcp_env["AUTH_SERVER_BASE"] = _asb
        _mbk = (getattr(settings, "lobster_mcp_billing_internal_key", None) or "").strip()
        if not _mbk:
            _mbk = (os.environ.get("LOBSTER_MCP_BILLING_INTERNAL_KEY") or "").strip()
        if _mbk:
            _mcp_env["LOBSTER_MCP_BILLING_INTERNAL_KEY"] = _mbk
        _csu = (getattr(settings, "capability_sutui_mcp_url", None) or "").strip()
        if _csu:
            _mcp_env["CAPABILITY_SUTUI_MCP_URL"] = _csu
