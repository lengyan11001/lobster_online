...
zip -r "$OUT" \
  ...
  -x "*.pyc" "*__pycache__*" "*.db" "machine_identity.json" "*/machine_identity.json" "*/*/machine_identity.json" "openclaw/identity/*" "openclaw/workspace/*" "openclaw/.env" "browser_data/*" "assets/*" "static/hifly_previews/*" "sutui_config.json" \
  -x "*probe_three_out*" \
  -x "*.DS_Store" \
  2>/dev/null || true
