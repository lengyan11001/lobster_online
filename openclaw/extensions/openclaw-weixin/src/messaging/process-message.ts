...
/**
 * lobster_online：本机 OpenClaw 调 mcp-gateway 时注入微信好友 ID，供龙虾后端从 .weixin_openclaw_peers.json 取对应用户 JWT 扣费。
 * 注意：依赖单账号内消息顺序处理；多会话并发极罕见，若并发请改用 OpenClaw 官方扩展字段（如有）。
 */
async function withLobsterWeixinMcpBillingHeader<T>(
  fromUserId: string,
  fn: () => Promise<T>,
): Promise<T> {
  const id = (fromUserId || "").trim();
  if (!id) return fn();
  const g = globalThis as typeof globalThis & { fetch: typeof fetch };
  const origFetch = g.fetch.bind(g) as typeof fetch;
  g.fetch = (async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (lower.includes("mcp-gateway")) {
      if (input instanceof Request) {
        const h = new Headers(input.headers);
        h.set("X-Lobster-Weixin-From-User-Id", id);
        const req2 = new Request(input, { headers: h });
        return origFetch(req2, init);
      }
      const h = new Headers(init?.headers);
      h.set("X-Lobster-Weixin-From-User-Id", id);
      return origFetch(input as RequestInfo, { ...init, headers: h });
    }
...
export async function processOneMessage(
  ...
  try {
    await deps.channelRuntime.reply.withReplyDispatcher({
      dispatcher,
      run: () =>
        withLobsterWeixinMcpBillingHeader(String(full.from_user_id ?? finalized.From ?? "").trim(), () =>
