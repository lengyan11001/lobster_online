 ...
 * - /myid                   返回当前私聊在助手侧的好友 ID（用于 lobster POST /auth/persist-weixin-openclaw-peer 绑定扣费）
 */
...
export async function handleSlashCommand(
  ...
  try {
    switch (command) {
      ...
      case "/myid": {
        const uid = (ctx.to || "").trim();
        await sendReply(
          ctx,
          uid
            ? `你的助手侧好友 ID：\n${uid}\n\n在本站网页登录后，用 Bearer 调用 POST /auth/persist-weixin-openclaw-peer，body: {"weixin_user_id":"${uid}"} 即可绑定扣费。`
            : "无法解析好友 ID（ctx.to 为空）。",
        );
        return { handled: true };
      }
