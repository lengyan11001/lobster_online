(function initLobsterI18n() {
  ...
  var CURATED_EN = {
    ...
    '审核稿出几次（1～10）': 'Number of review drafts (1-10)',
    ...
    '抖音私信识别手机号后加微信好友': 'Add WeChat friend after Douyin DM detects a phone number',
    '不勾选时本机不动微信：识别到的手机号上报到服务端账号池，由同账号的机器上加好友。': 'When unchecked this device leaves WeChat alone: detected phone numbers are reported to the account-level server pool, and another device on the same account adds them.',
    '个微加好友目标来源': 'WeChat add-friend target source',
    '本机导入名单（节点自己带的手机号/微信号）': 'Local list (phones / WeChat IDs carried by this node)',
    '上级抖音私信结果（同一台机器刚跑完的私信接管）': 'Upstream Douyin DM result (DM run just finished on this device)',
    '服务端上报池（同账号跨机器共用）': 'Server-reported pool (shared by this account across devices)',
    '每轮领取数量': 'Items claimed per round',
    ...
    '工作台状态': 'Workbench status', '测试模式': 'Test mode', '当前未强制校验登录态': 'Sign-in is not enforced',
