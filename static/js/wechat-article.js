(function() {
  ...
  function renderGenerated(data) {
    ...
    if (meta) {
      ...
      var imageText = image && image.enabled
        ? (generatedCount ? ('已插入 ' + generatedCount + '/' + requestedCount + ' 张自动配图。') : '文章已生成。')
        : '未开启自动配图。';
  ...
  function createDraft() {
    ...
    fetch(apiUrl('/api/wechat-article/drafts'), {
      ...
      .then(function(r) { return r.json().then(function(d) { return { ok: r.ok, data: d }; }); })
      .then(function(x) {
        if (!x.ok) throw new Error((x.data && x.data.detail) || '推送失败');
        var draft = (x.data && x.data.draft) || {};
        if (x.data && x.data.pushed) {
          showMsg('已推送到公众号草稿箱：' + (draft.media_id || ''), false);
        } else {
          showMsg((x.data && x.data.message) || '文章已保存到公众号文章页面，可稍后再次推送。', false);
        }
        loadDrafts();
      })
      .catch(function(err) {
