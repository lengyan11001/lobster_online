...
def test_website_ota_paths_carry_brand_assets(tmp_path):
    """2026-09-20 build 341 事故回归：常规网站 OTA 必须带上 static 根目录的品牌资源图，
    否则客户端按 manifest 对账时会把本地 OEM 品牌图删掉（左上角 logo 变空白）。"""
    static = tmp_path / "static"
    static.mkdir()
    (static / "daka_32.png").write_bytes(b"\x89PNG")
    (static / "daka_header_partner.jpg").write_bytes(b"\xff\xd8")
    (static / "index.html").write_text("<html></html>", encoding="utf-8")

    assets = packer.brand_asset_rel_paths(tmp_path)
    assert "static/daka_32.png" in assets
    assert "static/daka_header_partner.jpg" in assets
    assert "static/index.html" not in assets

    paths = packer.with_brand_assets(packer.WEBSITE_OTA_PATHS, tmp_path)
    assert "static/daka_32.png" in paths
    assert paths[-2:] == ("static/client_version.json", "CLIENT_CODE_VERSION.json")


def test_incremental_apply_writes_listed_brand_assets_back(tmp_path, monkeypatch):
    """被上一次 OTA 删掉的品牌图，只要新包把它列进 manifest，就必须能被写回来。"""
    monkeypatch.setattr(updater, "ROOT", tmp_path)
    static = tmp_path / "static"
    static.mkdir()
    bundle_static = tmp_path / "bundle" / "static"
    (bundle_static / "css").mkdir(parents=True)
    (bundle_static / "css" / "app.css").write_text("code", encoding="utf-8")
    (bundle_static / "daka_32.png").write_bytes(b"\x89PNG-fresh")

    stats = updater._sync_tree_incremental(
        bundle_static,
        static,
        preserve_rels=updater._PRESERVED_STATIC_REL_PATHS,
    )

    assert (static / "css" / "app.css").read_text(encoding="utf-8") == "code"
    assert (static / "daka_32.png").read_bytes() == b"\x89PNG-fresh"
...
def test_default_ota_is_encrypted_website_only(monkeypatch, tmp_path):
    ...
    (tmp_path / "static" / "daka_32.png").write_bytes(b"\x89PNG")
    ...
    with zipfile.ZipFile(out) as archive:
        ...
        # OEM 品牌资源图必须随常规 OTA 走，否则客户端对账时会把本地品牌图删掉
        assert "static/daka_32.png" in names
        # 2026-09-20 起常规网站 OTA 也带 skills（只排除体积大/解压慢的，见 _OTA_SKIP_SKILL_ROOTS）
        assert "skills/should_not_ship/SKILL.md" in names
